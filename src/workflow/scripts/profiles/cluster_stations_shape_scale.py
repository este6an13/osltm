"""
Shape + Scale Clustering of Station Profiles

This script performs shape+scale clustering on station mean arrival profiles.
- Computes mean profiles for each station (fixed day type)
- Applies log-transform (log(x + 1) to handle zeros)
...
def log_transform(profiles_df: pd.DataFrame) -> pd.DataFrame:
    """
    Apply log-transform to profiles: log(x + 1).
    ...
    """
    log_profiles = profiles_df.copy()
    log_profiles = np.log1p(log_profiles)  # log(x + 1)
    return log_profiles
...
def plot_cluster_profiles(
    ...
    for cluster_id in range(1, n_clusters + 1):
        ...
        ax.set_title(
            f"Cluster {cluster_id} (n={len(cluster_stations)})", fontsize=12, fontweight="bold"
        )
    ...
    fig.suptitle(
        f"Shape + Scale Clustering: Station Profiles ({day_type_label})\n"
        f"{count_type.capitalize()} - Log-Transform + Ward Clustering",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
    ...
    if output_dir:
        ...
        filename = output_dir / f"cluster_shape_scale_{day_type}_{count_type}.png"
...
def run_shape_scale_clustering(
    ...
    Returns:
        Tuple of (log_profiles_df, cluster_labels)
    """
    # Load parameters
    ...
    if station_codes is not None:
        ...
        if missing:
            print(f"⚠️  Warning: Some specified stations have no data: {sorted(missing)}")
    ...
    # Apply log-transform
    print("\n🔄 Applying log-transform (log(x + 1))...")
    log_profiles = log_transform(mean_profiles_df)
    print("   Profiles log-transformed")

    # Perform hierarchical clustering
    print(f"\n🔗 Performing hierarchical clustering (Ward, {n_clusters} clusters)...")
    cluster_labels = perform_hierarchical_clustering(
        log_profiles.values, n_clusters, method="ward"
    )
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    summary_rows = []
    for i, station_code in enumerate(log_profiles.index):
        station_name = station_names.get(station_code, station_code)
        summary_rows.append(
            {
    ...
    # Add cluster sizes
    unique_clusters_final, counts_final = np.unique(cluster_labels, return_counts=True)
    cluster_size_map = dict(zip(unique_clusters_final, counts_final))
    summary_df["cluster_size"] = summary_df["cluster"].map(cluster_size_map)

    csv_path = (
        output_dir
        / f"clustering_shape_scale_{day_type}_{count_type}_{n_clusters}clusters.csv"
    )
    summary_df.to_csv(csv_path, index=False)
    print(f"💾 Saved clustering summary to {csv_path}")

    return log_profiles, cluster_labels
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Shape + scale clustering of station arrival profiles"
    )
    parser.add_argument(
        "--count_type",
    ...
    results = run_shape_scale_clustering(
