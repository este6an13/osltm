...
# ---------------------------------------------------------------------------
# Option D — Trend (Derivative) Scaling
# ---------------------------------------------------------------------------

@dataclass
class TrendAdaptation:
    """
    Derivative-based correction: uses the recent sequence of observation ratios
    to compute a linear trend (slope) over time, and extrapolates that direction
    into the look-ahead window.
    """

    prior_means: np.ndarray
    history_size: int = 4   # Number of recent bins to consider for trend

    _ratios: np.ndarray = field(init=False)
    _obs_history: list[tuple[int, float]] = field(default_factory=list)
    ...
    def update(self, bin_indices: np.ndarray, observed_counts: np.ndarray) -> None:
        ...
        for idx, n_obs in zip(bin_indices, observed_counts):
            if 0 <= idx < K:
                model_val = max(self.prior_means[idx], 1e-3)
                new_ratio = float(n_obs) / model_val
                # Save to history for trend computation
                self._obs_history.append((idx, new_ratio))
                self._ratios[idx] = new_ratio
        
        # Keep only recent history
        if len(self._obs_history) > self.history_size:
            self._obs_history = self._obs_history[-self.history_size:]

    def _get_trend_params(self) -> tuple[float, float]:
        """Compute (slope, intercept) from recent observation history."""
        if len(self._obs_history) < 2:
            return 0.0, self._obs_history[-1][1] if self._obs_history else 1.0

        x = np.array([item[0] for item in self._obs_history])
        y = np.array([item[1] for item in self._obs_history])
        ...
        slope = np.sum(dx * (y - my)) / ss_xx
        intercept = my - slope * mx
        
        return slope, intercept

    def corrected_forecast(self, model_forecast: np.ndarray, bin_start: int) -> np.ndarray:
        K = len(model_forecast)
        result = model_forecast.copy().astype(float)
        
        slope, intercept = self._get_trend_params()
        
        for i, val in enumerate(model_forecast):
            k = bin_start + i
            projected_ratio = slope * k + intercept
            # Clamp projection to avoid wild values
            projected_ratio = float(np.clip(projected_ratio, 0.1, 10.0))
            result[i] = val * projected_ratio
...
AdaptationMethod = Literal["bayesian", "multiplicative", "hawkes_kappa", "trend"]
...
) -> BayesianAdaptation | MultiplicativeAdaptation | TrendAdaptation | HawkesKappaAdaptation:
    if method == "bayesian":
        ...
        return TrendAdaptation(prior_means=prior_means)
