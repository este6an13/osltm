...
def create_session(
    ...
    Create and fully initialize a RealtimeSession:
      1. Infer day_type from DB.
      2. Load model parameters for each station.
      3. Pre-generate model events.
      4. Load real events from daily CSV.
      5. Initialize adaptation state.
    """
    # 1. Day type
    day_type = query_day_type(date_str)
    if day_type is None:
        # Date not in DB — we can still simulate, just no real data
        day_type = "WD"   # fallback; UI should warn
